...
func (s *Server) handleDashboardAPI(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(s.dashboardSnapshot())
}
...
func (s *Server) dashboardSnapshot() dashboardResponse {
	services := []dashboardService{
		{ID: "s3", Name: "S3", Provider: "AWS", Description: "버킷과 객체 저장 상태", Status: "READY", Resources: []dashboardResource{}},
		{ID: "sqs", Name: "SQS", Provider: "AWS", Description: "큐와 대기 메시지 상태", Status: "READY", Resources: []dashboardResource{}},
		{ID: "dynamodb", Name: "DynamoDB", Provider: "AWS", Description: "테이블 스키마와 저장 아이템 상태", Status: "READY", Resources: []dashboardResource{}},
		{ID: "sts", Name: "STS", Provider: "AWS", Description: "로컬 AWS 호출자 identity", Status: "READY", Resources: []dashboardResource{}},
		{ID: "gcs", Name: "Cloud Storage", Provider: "GCP", Description: "버킷과 객체 저장 상태", Status: "READY", Resources: []dashboardResource{}},
		{ID: "pubsub", Name: "Pub/Sub", Provider: "GCP", Description: "토픽, 구독과 미확인 메시지", Status: "READY", Resources: []dashboardResource{}},
		{ID: "firestore", Name: "Firestore", Provider: "GCP", Description: "저장된 문서 메타데이터", Status: "READY", Resources: []dashboardResource{}},
		{ID: "secrets", Name: "Secret Manager", Provider: "GCP", Description: "값을 제외한 Secret과 버전 상태", Status: "READY", Resources: []dashboardResource{}},
		{ID: "kms", Name: "Cloud KMS", Provider: "GCP", Description: "키링, 키와 버전 상태", Status: "READY", Resources: []dashboardResource{}},
		{ID: "iam", Name: "IAM Credentials", Provider: "GCP", Description: "개인키를 제외한 로컬 서비스 계정", Status: "READY", Resources: []dashboardResource{}},
		{ID: "fcm", Name: "FCM", Provider: "GCP", Description: "외부 발송 없이 캡처된 요청", Status: "READY", Resources: []dashboardResource{}},
		{ID: "metadata", Name: "Compute Metadata", Provider: "GCP", Description: "로컬 프로젝트와 서비스 계정 identity", Status: "READY", Resources: []dashboardResource{}},
		{ID: "vertex", Name: "Vertex AI", Provider: "GCP", Description: "모델 목록과 로컬 생성 호출 상태", Status: "READY", Resources: []dashboardResource{}},
	}
	...
	for i := range services {
		...
		resourceCount += len(services[i].Resources)
	}
	return dashboardResponse{
		Project:     s.projectID,
		GeneratedAt: time.Now().UTC(),
		Summary: dashboardSummary{
			ServiceCount: len(services), ResourceCount: resourceCount, MessageCount: messageCount,
		},
		Services: services,
	}
}
