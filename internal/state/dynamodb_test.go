...
func TestDynamoDBPersistsAndWorkloadResetPreservesTable(t *testing.T) {
	...
	if _, _, err := store.DynamoPutItem("podo-notification", item, nil); err != nil {
		t.Fatal(err)
	}
	...
	stored, exists, err := reopened.DynamoGetItem("podo-notification", DynamoItem{"pk": {S: &pk}, "sk": {S: &sk}})
	...
	table, err := reopened.DynamoTable("podo-notification")
